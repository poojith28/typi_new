...
To use evolving representations instead of fixed precomputed features, switch
`ACTIVE_LEARNING.IDPC_FEATURE_SOURCE` from `precomputed` to `model`. In that mode,
IDProbCover extracts the current penultimate embeddings from the classifier at each
AL round, recomputes local ID and neighborhoods on those embeddings, and disables
the fixed-geometry cache automatically. You can also set
`ACTIVE_LEARNING.IDPC_FEATURE_EMA` in `[0, 1)` to smooth features across rounds,
with the running state stored in `ACTIVE_LEARNING.IDPC_FEATURE_STATE`.
